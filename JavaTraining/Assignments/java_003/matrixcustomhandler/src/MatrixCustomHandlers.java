...
public class MatrixCustomHandlers {
	...
	static void printToFile(String arr[][]) {
		...
		} catch (FileNotFoundException | InvalidDieException e) {
	...
	static void validDie(int x) throws InvalidDieException {
		if (x <= 5 && x >=0) {
			
		} else {
			throw new InvalidDieException("This is not a possible dice combination!");
		}
	}
	static void validChoice(String s) throws IncorrectChoiceException {
		if (s.equalsIgnoreCase("a") || s.equalsIgnoreCase("b") || s.equalsIgnoreCase("c")) {
			
		} else {
			throw new IncorrectChoiceException("This is an invalid user choice!");
		}
	}
